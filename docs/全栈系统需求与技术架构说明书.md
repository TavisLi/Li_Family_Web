...
### 5.6 HomeConfig (全域首页动态配置模型 - Global)
为了保持代码与内容的解耦，首页的欢迎语、轮播预设图以及动态宣发内容需透过 Payload Globals 进行管理。严禁在前端硬编码首页文案。
...
export const HomeConfig: GlobalConfig = {
  slug: 'home-config',
  access: {
    read: () => true, // 首页配置必须公开可读
  },
  fields: [
    { name: 'heroTitle', type: 'text', localized: true, required: true, defaultValue: 'Welcome to Web Li' },
    { name: 'heroSubtitle', type: 'textarea', localized: true },
    { name: 'heroBackground', type: 'relationship', relationTo: 'media', required: false, description: '首页首屏中立状态下的备用高画质背景图' },
    { 
      name: 'featuredTravel', 
      type: 'relationship', 
      relationTo: 'travel-projects', 
      hasMany: false,
      description: '管理员可在此手动置顶当前最重要的家庭旅游计划'
    },
    { 
      name: 'announcement', 
      type: 'text', 
      localized: true, 
      description: '全站置顶跑马灯公告 (可选)' 
    }
  ],
}
```
