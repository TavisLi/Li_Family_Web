# Web Li 家庭入口网站 - 全栈系统需求与技术架构说明书

---
title: "全栈系统需求与技术架构说明书"
author: "Tavis Li"
date: "2026-06-09"
---

版本: Version 3.3 (整合 V2 微交互与 V3 数据源规范)  
更新日期: 2026-06-09  
...
│   └── api.ts                # API 請求/響應類型
├── public/                   # 靜態資源
├── content-source/           # 👈 建议创建的专属内容文件夹（与代码完全解耦）
│   ├── profiles/              # 存放家庭成员个人履历数据
│   │   ├── lynn_resume.md
│   │   └── tavis_resume.md
│   └── travels/               # 存放所有旅游项目的行程与心得数据
│       ├── 201307海南岛8日.md
│       ├── 202308东澳全览9日.md
│       └── 202607重庆长江三峡8日.md
└── .env.example              # 環境變數範
...
### 4.2 全域原始數據隔離與内容源數據（content source）規範

為了確保全棧架構的純淨度，並為後續自動化數據灌錄（Data Seeding）打下基礎，系統強制執行「程式碼與原始內容完全解耦」原則：

1. **專屬目錄設立**：
   於專案根目錄下設立獨立的 `content-source/` 資料夾，嚴禁將原始 Markdown（.md）內容文件混入 `app/`、`components/` 或 `payload/` 等程式碼核心目錄。
   
2. **目錄結構定義**：
   - `content-source/profiles/`：存放家庭成員的原始履歷與個人檔案（如 `lynn_resume.md`, `tavis_resume.md`）。
   - `content-source/travels/`：存放所有旅遊項目的行程、規劃與心得底稿（如 `202308东澳全览9日.md`, `202607重庆长江三峡8日.md`）。

3. **架構優勢與嚴格邊界（AI 必須遵守）**：
   - **單一事實來源（SSOT）**：`content-source/` 內的 Markdown 文件為系統初始化的唯一文本依據。資料庫（Supabase）重置時，系統必須能通過腳本一鍵重新讀取此目錄進行播種。
   - **動態擴充性**：未來新增旅遊項目或个人档案時，用戶只需將新的 `.md` 文件放入 `content-source/travels/` 与 `content-source/profiles/`，系統後續的自動化腳本應具備遍歷該目錄並動態新增資料庫條目的能力，前端 UI 須自動感知，嚴禁手写死路由。
   - **Token 消耗優化**：AI 代理人在進行前端組件編寫時，應直接引用 Payload 自動生成的強類型（`payload-types.ts`），嚴禁在前端組件中重複掃描解析 `content-source/` 內的大型文本文件，以極大化節省 Token 消耗。
